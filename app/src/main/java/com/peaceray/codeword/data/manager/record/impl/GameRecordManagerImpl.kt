...
class GameRecordManagerImpl @Inject constructor(
            ...
            recordMutex.withLock {
                ...
                if (previousOutcome == null) {
                    ...
                    dbHelper.writableDatabase.insertWithOnConflict(
                        GameRecordContract.GameOutcomeEntry.TABLE_NAME,
                        "",
                    ...
                    dbHelper.writableDatabase.update(
                        GameRecordContract.GameOutcomeEntry.TABLE_NAME,
                        putValues(ContentValues(), gameSaveData, secret),
                        "${GameRecordContract.GameOutcomeEntry.COLUMN_NAME_GAME_UUID} = ?",
                ...
                if (
                    ...
                    streakCV.put(GameRecordContract.PlayerStreakEntry.COLUMN_NAME_GAME_TYPE, gameType.toString())
                    dbHelper.writableDatabase.insertWithOnConflict(
                        GameRecordContract.PlayerStreakEntry.TABLE_NAME,
                        "",
                        streakCV,
                        SQLiteDatabase.CONFLICT_IGNORE
                    )
                    ...
                    if (rows != 1) Timber.e("Failed to create or update ${GameRecordContract.PlayerStreakEntry.TABLE_NAME} row")
                }
            }
        }
    }
    ...
    private fun recordGamePerformance(
        ...
        listOf(gameType, null).forEach {
            ...
            val gameTypeName = it?.toString() ?: GameRecordContract.GameTypePerformanceEntry.COLUMN_VALUE_GAME_TYPE_ALL

            val performanceCV = readGameTypePerformance(gameTypeName, gameSaveData.setup.daily, solver.name, evaluator.name, arrayOf(
                BaseColumns._ID,
                GameRecordContract.GameTypePerformanceEntry.COLUMN_NAME_WINNING_TURN_COUNTS,
                GameRecordContract.GameTypePerformanceEntry.COLUMN_NAME_LOSING_TURN_COUNTS,
                GameRecordContract.GameTypePerformanceEntry.COLUMN_NAME_FORFEIT_TURN_COUNTS
            ))
            ...
            if (performanceCV == null) {
                ...
                dbHelper.writableDatabase.insert(
                    GameRecordContract.GameTypePerformanceEntry.TABLE_NAME,
                    "",
        ...
        return withContext(ioDispatcher) {
            GameRecordContract.GameOutcomeEntry.run { readGameOutcome(uuid)?.let {
                GameOutcome(
                    ...
                    evaluator = GameSetup.Evaluator.valueOf(it.getAsString(
                        COLUMN_NAME_EVALUATOR_ROLE
                    )),
        ...
        return withContext(ioDispatcher) {
            GameRecordContract.GameTypePerformanceEntry.run {
                ...
                readGameTypePerformance(type, false, solver.name, evaluator.name, columns)?.let {
                    record.winningTurnCounts.resetFromString(it.getAsString(
                        COLUMN_NAME_WINNING_TURN_COUNTS
                    ))
                    record.losingTurnCounts.resetFromString(it.getAsString(
                        COLUMN_NAME_LOSING_TURN_COUNTS
                    ))
                    record.forfeitTurnCounts.resetFromString(it.getAsString(
                        COLUMN_NAME_FORFEIT_TURN_COUNTS
                    ))
                }

                readGameTypePerformance(type, true, solver.name, evaluator.name, columns)?.let {
                    record.winningTurnCounts += IntHistogram.fromString(it.getAsString(
                        COLUMN_NAME_WINNING_TURN_COUNTS
                    ))
                    record.losingTurnCounts += IntHistogram.fromString(it.getAsString(
                        COLUMN_NAME_LOSING_TURN_COUNTS
                    ))
                    record.forfeitTurnCounts += IntHistogram.fromString(it.getAsString(
                        COLUMN_NAME_FORFEIT_TURN_COUNTS
                    ))
                }
                record
    ...
    private fun readGameOutcome(uuid: UUID, columns: Array<String> = GameRecordContract.GameOutcomeEntry.COLUMNS): ContentValues? {
            ...
            if (!it.moveToFirst()) null else {
                ...
                for (i in columns.indices) {
                    when (val column = columns[i]) {
                        GameRecordContract.GameOutcomeEntry.COLUMN_NAME_ROUNDS -> put(values, column, it.getIntOrNull(i))
                        GameRecordContract.GameOutcomeEntry.COLUMN_NAME_RECORDED_AT -> put(values, column, it.getLongOrNull(i))
    ...
    private fun readGameTypePerformance(
        ...
        val select = mutableListOf(
            "${GameRecordContract.GameTypePerformanceEntry.COLUMN_NAME_GAME_TYPE} = ?",
            "${GameRecordContract.GameTypePerformanceEntry.COLUMN_NAME_DAILY} = ?",
            "${GameRecordContract.GameTypePerformanceEntry.COLUMN_NAME_SOLVER_ROLE} = ?",
            "${GameRecordContract.GameTypePerformanceEntry.COLUMN_NAME_EVALUATOR_ROLE} = ?"
        )
    ...
    private fun readPlayerStreak(
        ...
        return dbHelper.readableDatabase.query(
            GameRecordContract.PlayerStreakEntry.TABLE_NAME,
            columns,
            "${GameRecordContract.PlayerStreakEntry.COLUMN_NAME_GAME_TYPE} = ?",
