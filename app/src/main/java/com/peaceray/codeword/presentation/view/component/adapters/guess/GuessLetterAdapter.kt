...
class GuessLetterAdapter @Inject constructor(
    ...
    // appearance details
    private val styleAppearance: MutableMap<ItemStyle, GuessLetterAppearance> = mutableMapOf()
    init {
        val context = layoutInflater.context
        styleAppearance[ItemStyle.LETTER_MARKUP] = GuessLetterMarkupAppearance(context, cellLayout[ItemStyle.LETTER_MARKUP] as GuessLetterCellLayout)
        styleAppearance[ItemStyle.LETTER_CODE] = GuessLetterCodeAppearance(context, cellLayout[ItemStyle.LETTER_CODE] as GuessLetterCellLayout)
    }
    ...
    fun setCellLayout(itemStyle: ItemStyle, cellLayout: CellLayout?) {
        ...
        when (itemStyle) {
            ItemStyle.LETTER_MARKUP -> {
                ...
                styleAppearance[itemStyle] = GuessLetterMarkupAppearance(layoutInflater.context, layout)
            }
            ItemStyle.LETTER_CODE -> {
                require(layout is GuessLetterCellLayout)
                styleAppearance[itemStyle] = GuessLetterCodeAppearance(layoutInflater.context, layout, _codeCharacters)
            }
    ...
    fun setCodeCharacters(codeCharacters: Iterable<Char>) {
        ...
        styleAppearance[ItemStyle.LETTER_CODE] = GuessLetterCodeAppearance(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (style) {
            ItemStyle.LETTER_MARKUP -> GuessLetterViewHolder(cellView, colorSwatchManager, styleAppearance[style]!!)
            ItemStyle.LETTER_CODE -> GuessLetterViewHolder(cellView, colorSwatchManager, styleAppearance[style]!!)
            ItemStyle.AGGREGATED_PIP_LINE -> AggregatedConstraintViewHolder(cellView, layoutInflater, colorSwatchManager)
            ItemStyle.AGGREGATED_PIP_CLUSTER -> ConstraintPipGridViewHolder(cellView, colorSwatchManager)
    ...
    override fun onBindGuessToViewHolder(
        ...
        when(holder) {
            is GuessLetterViewHolder -> {
                ...
                holder.appearance = styleAppearance[style]!!
                holder.bind(bindLetter)
            }
            is AggregatedConstraintViewHolder -> holder.bind(guess)
            is ConstraintPipGridViewHolder -> holder.bind(guess)
